...
The EnOS Stream Data Processing Service provides the following templates for calculating electric energy data.
...
When creating a stream data processing job, select one of the above templates to calculate electric energy production or consumption.
...
Follow the steps below to complete the configuration when creating a stream data processing job based on the **Electric Energy Cal by Instant Power** template.
